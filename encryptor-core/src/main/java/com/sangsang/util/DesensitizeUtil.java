...
public class DesensitizeUtil {
    ...
    //缓存脱敏对象，避免重复创建大量对象
    private final static Map<Class, DesensitizeStrategy> instanceMap = new HashMap<>();
     ...
     **/
    public static final <T> String desensitize(Class<? extends DesensitizeStrategy> dClass,
                                               String cleartext,
                                               T t) throws IllegalAccessException, InstantiationException {
        //1.避免重复反射创建对象，这里缓存脱敏实现方法对象
        DesensitizeStrategy desensitizeInstance = instanceMap.get(dClass);
