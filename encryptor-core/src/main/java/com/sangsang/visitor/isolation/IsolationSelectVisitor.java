    ...
    public void visit(PlainSelect plainSelect) {
        ...
        for (Map.Entry<String, Set<FieldInfoDto>> fieldTableEntry : fieldTableMap.entrySet()) {
            for (FieldInfoDto fieldInfo : fieldTableEntry.getValue()) {
                //4.3.1 判断当前表是否存在隔离字段
                DataIsolationDto dataIsolationDto = IsolationCache.getIsolationData(fieldInfo.getSourceTableName());
                //4.3.2 此表不存在隔离字段，跳过这张表
                if (dataIsolationDto == null) {
                    break;
                }
                //4.3.3当前字段不是直接来自真实表的，跳过
                if (!fieldInfo.isFromSourceTable()) {
                    continue;
                }
                //4.3.4 当前字段和数据隔离字段不同，跳过
                if (!StringUtils.equalIgnoreFieldSymbol(fieldInfo.getColumnName(), dataIsolationDto.getField())) {
                    continue;
                }
                //4.3.5 当前数据隔离值为空，跳过 （拼凑个为空的条件，sql肯定执行出问题，所以打个警告日志后跳过）
                String isolationValue = IsolationCache.getIsolationInterface(dataIsolationDto.getisolationClass()).getIsolationData();
                if (StringUtils.isBlank(isolationValue)) {
                    log.warn("【isolation】当前表的权限隔离字段获取为空，请留意是否正常 tableName:{} isolationClass:{}", fieldInfo.getSourceTableName(), dataIsolationDto.getisolationClass());
                    break;
                }
                //4.3.6开拼
                Expression isolationExpression = IsolationCache.buildIsolationExpression(dataIsolationDto.getField(), fieldTableEntry.getKey(), dataIsolationDto.getRelation(), isolationValue);
